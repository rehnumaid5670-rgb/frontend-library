...
function LibraryMedia() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [ebooksRes, audiobooksRes] = await Promise.all([
        axios.get("https://backend1-ax6k.onrender.com/api/ebook"),
        axios.get("https://backend1-ax6k.onrender.com/api/audiobook"),
      ]);
  ...
  const deleteItem = async (id, type) => {
    ...
    try {
      await axios.delete(`https://backend1-ax6k.onrender.com/api/${type}/${id}`);
      fetchData();
    } catch (err) {
      console.error(err);
    }
